...
BASE_DIR = find_project_root(APP_DIR)
DEFAULT_EXCEL = BASE_DIR / "data" / "EM_RV_Data_Updated_Apr_03.xlsx"
...
def compute_half_life(residuals: pd.Series):
    try:
        r = residuals.dropna()
        if len(r) < 10:
            return np.nan
        ...
        for j in range(i + 1, n):
            ...
            if mode == "coint":
                ...
                if w.shape[0] < min_periods:
                    continue
                try:
                    X = sm.add_constant(sub["y"].astype(float), has_constant="add")
                    yv = sub["x"].astype(float)
                    model = sm.OLS(yv, X).fit()
                    params = model.params
                    a = float(params.get("const", 0.0))
                    b = float(params.get("y", 0.0))
        
                    resid_window = (w["x"].astype(float) - (a + b * w["y"].astype(float))).astype(float)
                    
                    resid_full = sub["x"] - (a + b * sub["y"])
                    resid_full = resid_full.dropna()
                    
                    try:
                        adf_pval = float(adfuller(resid_full.values)[1])
                    except Exception:
                        adf_pval = np.nan


                    # --- NEW: Half-life ---
                    hl = compute_half_life(resid_full)
...
def _iter_pairs_stats(
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            ...
            yield x, y, buy, sell, cur_sb, mu_sb, sd, z, z_abs, adf_pval, hl                               
...
tab_raw, tab_coint = st.tabs(["Raw Z", "Cointegration Z (proxy)"])
